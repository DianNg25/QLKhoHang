 ...
 * @author Nguyen
 */
public class Model_Edit_Product1 extends javax.swing.JPanel {
...
//                fillComboBox();
    ...
    private void updateProduct() {
        // Kiểm tra lỗi: Các trường không được để trống
        ...
        String mau = cboMau.getSelectedItem().toString().trim();
        ...
        // Chuỗi lệnh SQL để kiểm tra mã sản phẩm
        String sqlCheck = "SELECT COUNT(*) FROM Products WHERE productID = ?";

        // Kiểm tra mã sản phẩm trong cơ sở dữ liệu
        try (java.sql.Connection conn = XJdbc.getConnection(); java.sql.PreparedStatement stmtCheck = conn.prepareStatement(sqlCheck)) {
            ...
            JOptionPane.showMessageDialog(this, "Lỗi khi kiểm tra mã sản phẩm: " + e.getMessage(), "Lỗi", JOptionPane.ERROR_MESSAGE);
            return;
        }

        // Chuỗi lệnh SQL để cập nhật sản phẩm
        String sqlUpdate = "UPDATE Products SET productName = ?, price = ?, color = ?, weight = ?, status = ? WHERE productID = ?";

        // Cập nhật sản phẩm trong cơ sở dữ liệu
        try (java.sql.Connection conn = XJdbc.getConnection(); java.sql.PreparedStatement stmtUpdate = conn.prepareStatement(sqlUpdate)) {
            stmtUpdate.setString(1, tenSP);
            stmtUpdate.setDouble(2, gia);
            stmtUpdate.setString(3, mau);
            stmtUpdate.setString(4, loai);
            stmtUpdate.setString(5, trangThai);
            stmtUpdate.setString(6, maSP);
    ...
    private void initComponents() {
        ...
        rdoMini.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                rdoMiniActionPerformed(evt);
            }
        });
    ...
    private void rdoMiniActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_rdoMiniActionPerformed
        // TODO add your handling code here:
    }//GEN-LAST:event_rdoMiniActionPerformed
    ...
    public void setProductData(ProductsTable productsTable) {
        if (productsTable == null) {
            ...
            txtMaSanPham.setText(productsTable.getProductID());
            txtTenSanPham.setText(productsTable.getProductName());
            txtGia.setText(String.valueOf(productsTable.getPrice()));
            cboMau.setSelectedItem(productsTable.getColor());
            // Cập nhật loại sản phẩm
            String weight = productsTable.getWeight();
            if ("Mini".equals(weight)) {
                rdoMini.setSelected(true);
            } else if ("Nhỏ".equals(weight)) {
                rdoNho.setSelected(true);
            } else if ("Vừa".equals(weight)) {
                rdoVua.setSelected(true);
            } else if ("Lớn".equals(weight)) {
                rdoLon.setSelected(true);
            }
        }
